...
parser.add_argument('--input-dir', required=True, help='Directory containing exported PR data (individual prs)')
...
def main():
    ...
    # Create or update an index file mapping URLs to filenames
    ...
    if os.path.exists(index_file):
        try:
            ...
            logger.info(f"Loaded existing image index with {len(existing_index)} entries")
        except Exception as e:
            logger.error(f"Error loading existing index file: {str(e)}")

    # Merge existing and new indices (new entries take precedence)
    merged_index = {**existing_index, **all_images}

    with open(index_file, 'w', encoding='utf-8') as f:
        json.dump(merged_index, f, indent=2)

    logger.info(f"Created/updated image index at {index_file} with {len(merged_index)} entries")
