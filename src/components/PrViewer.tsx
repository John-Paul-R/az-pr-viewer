  ...
  if (!prData) {
    return (
      <div className="error-container">
        <p className="error">
          No PR data found. Please select a PR file from the main page.
        </p>
      ...
      <div className="pr-card">
        <div className="pr-header">
          <div>
            ...
            <div>
              <span className={`status-badge ${getStatusClass(prData.status)}`}>
                {prData.status}
              </span>
            </div>
          </div>
          {prData.url && (
            <div className="external-link">
              <a href={prData.url} target="_blank" rel="noopener noreferrer">
                View in Browser
              </a>
            </div>
          )}
        ...
        <div className="pr-metadata">
          <div className="metadata-item">
            ...
            <span className="metadata-value">{prData.created_by}</span>
          </div>
          <div className="metadata-item">
          ...
          <div className="metadata-item">
            ...
            <span className="metadata-value">{prData.repository}</span>
          </div>
          <div className="metadata-item">
            <span className="metadata-label">Source Branch</span>
            <span className="metadata-value">{formatBranchName(prData.source_branch)}</span>
          </div>
          <div className="metadata-item">
            <span className="metadata-label">Target Branch</span>
            <span className="metadata-value">{formatBranchName(prData.target_branch)}</span>
          </div>
        </div>
        ...
        <div className="pr-branches">
          <div className="branch source">
            ...
            <span className="branch-name">{formatBranchName(prData.source_branch)}</span>
          </div>
          <div className="branch-arrow">→</div>
          <div className="branch target">
            <span className="label">Target:</span>
            <span className="branch-name">{formatBranchName(prData.target_branch)}</span>
          </div>
        </div>
        ...
        {prData.threads && prData.threads.length > 0 ? (
          <div className="threads-section">
            <h3>
              Activity (
              {prData.threads.filter((thread) => !thread.isDeleted).length}{" "}
              items)
            </h3>
            ...
            {[...prData.threads]
              ...
              .map((thread, threadIndex) => {
                ...
                return (
                  <div className="thread" key={`thread-${threadIndex}`}>
