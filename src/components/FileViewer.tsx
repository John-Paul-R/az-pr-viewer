      ...
      <div className="file-grid">
        ...
        {files.map((file) => (
          ...
          >
            ...
            <div className="file-cell file-title">{file.title || 'Unknown Title'}</div>
            <div className="file-cell">{file.author || 'Unknown Author'}</div>
            <div className="file-cell status-cell">
              <span className={`status-badge status-${file.status || 'unknown'}`}>
                {file.status || 'unknown'}
              </span>
            </div>
            <div className="file-cell">
              {file.creation_date ? new Date(file.creation_date).toLocaleDateString() : 'Unknown'}
            </div>
          </Link>
        ))}
      </div>
