...
export const ThreadFilter = ({ onFilterChange, currentFilter }: ThreadFilterProps) => {
  const allButtonRef = useRef<HTMLButtonElement>(null);
  const commentsButtonRef = useRef<HTMLButtonElement>(null);
  
  const handleKeyDown = (e: KeyboardEvent<HTMLButtonElement>, option: FilterOption) => {
  ...
  return (
    <div className="thread-filter" role="radiogroup" aria-label="Thread filter options">
      <div className="radio-button-group">
        ...
        <button
          ...
          className={`filter-button ${currentFilter === 'comments' ? 'active' : ''}`}
